...
const expectedDmg = "94987fd07929d17c4de2c2d2a7b7bb7864e0712056eac47ef049bbc2975541ac";
...
assert.equal(actualDownload, product + "/downloads/DropEdge-Free-2.6.dmg");
...
for (const pathname of ["styles.css", "assets/og.png", "assets/app-icon.png", "updates/appcast.xml", "updates/appcast-unified.xml", "updates/DropEdge-Free-2.6.md"]) {
...
for (const pathname of ["downloads/DropEdge-Free-2.6.dmg", "downloads/DropEdge-Latest.dmg"]) {
  const response = await request(product + "/" + pathname);
  assert.equal(response.status, 200);
...
if (partial.status === 206) {
  assert.deepEqual(partial.body, repoFile("dropedge/downloads/DropEdge-Free-2.6.dmg").subarray(0, 512));
} else {
  assert.equal(sha256(partial.body), expectedDmg);
}
console.log("PASS range request bytes (HTTP " + partial.status + ")");

const release25 = await request(product + "/downloads/DropEdge-Free-2.5.dmg");
assert.equal(release25.status, 200);
assert.equal(sha256(release25.body), "b5168f5bfe2e3325950e523b5bc0e55dde5aba6b7c1589eb0b4253fec924c7ef");
console.log("PASS version 2.5 remains available");

const release23 = await request(product + "/downloads/DropEdge-Free-2.3.dmg");
assert.equal(release23.status, 200);
assert.equal(sha256(release23.body), "ecf1acbedc3054993d8fddf0f1f7b8baf16d02a3c1415b1ec9fcbb4b75d77c2d");
console.log("PASS version 2.3 remains available");
...
assert.equal(sha256(priorRelease.body), priorReleaseDmg);
console.log("PASS version 2.1 remains available");

for (const pathname of ["downloads/DropEdge-Free-1.6-build12.dmg", "downloads/DropEdge-Free-1.6.dmg"]) {
  const previous = await request(product + "/" + pathname);
  assert.equal(previous.status, 200);
  assert.equal(sha256(previous.body), "be4107832d7adf05074cdfca947b25c6a709cd1f328a8ef735153a7a579cb230");
}
console.log("PASS version 1.6 remains available");
...
for (const pathname of ["downloads/DropEdge-Free-1.1.0-build8.dmg", "downloads/DropEdge-Free-1.1.0.dmg"]) {
  ...
  assert.equal(sha256(legacy.body), legacyDmg);
}
console.log("PASS legacy version DMGs remain available");
