...
const expectedDmg = "b5168f5bfe2e3325950e523b5bc0e55dde5aba6b7c1589eb0b4253fec924c7ef";
...
assert.equal(actualDownload, product + "/downloads/DropEdge-Free-2.5.dmg");
...
for (const pathname of ["styles.css", "assets/og.png", "assets/app-icon.png", "updates/appcast.xml", "updates/appcast-unified.xml", "updates/DropEdge-Free-2.5.md"]) {
...
for (const pathname of ["downloads/DropEdge-Free-2.5.dmg", "downloads/DropEdge-Latest.dmg"]) {
  const response = await request(product + "/" + pathname);
  assert.equal(response.status, 200);
...
if (partial.status === 206) {
  assert.deepEqual(partial.body, repoFile("dropedge/downloads/DropEdge-Free-2.5.dmg").subarray(0, 512));
} else {
