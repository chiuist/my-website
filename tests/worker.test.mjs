...
test("download range/conditional requests and binary streams are preserved", async () => {
  const response = new Response(new Uint8Array([0, 1, 255]), { status: 206, headers: { "Content-Type": "application/octet-stream", "Content-Range": "bytes 0-2/1536462", ETag: '"build8"' } });
  const env = assetsMock(response);
  const request = new Request(origin + "/downloads/DropEdge-Latest.dmg", { headers: { Range: "bytes=0-2", "If-Range": '"build8"' } });
  const result = await worker.fetch(request, env);
  assert.equal(result, response);
  assert.equal(env.requests[0].headers.get("Range"), "bytes=0-2");
  assert.equal(env.requests[0].headers.get("If-Range"), '"build8"');
...
test("not-modified responses do not redirect", async () => {
  const env = assetsMock(new Response(null, { status: 304, headers: { ETag: '"build8"' } }));
