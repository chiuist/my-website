...
for (const pathname of ["/styles.css", "/assets/og.png", "/downloads/DropEdge-Latest.dmg", "/downloads/DropEdge-Free-1.3-build9.dmg", "/downloads/DropEdge-Free-1.3.dmg"]) {
  test(`subdomain maps ${pathname} to DropEdge assets`, async () => {
    const env = assetsMock();
    const result = await worker.fetch(new Request(origin + pathname + "?v=9"), env);
    assert.equal(result, env.response);
    assert.equal(env.requests.length, 1);
    assert.equal(env.requests[0].url, origin + "/dropedge" + pathname + "?v=9");
  });
}
...
for (const [acceptLanguage, language, assetPath] of languageCases) {
  test(`homepage selects ${language} for ${acceptLanguage || "no browser language"}`, async () => {
    ...
    const result = await worker.fetch(new Request(origin + "/?v=9", { headers }), env);
    assert.equal(new URL(env.requests[0].url).pathname, assetPath);
    assert.equal(new URL(env.requests[0].url).search, "?v=9");
...
test("download range/conditional requests and binary streams are preserved", async () => {
  const response = new Response(new Uint8Array([0, 1, 255]), { status: 206, headers: { "Content-Type": "application/octet-stream", "Content-Range": "bytes 0-2/1611782", ETag: '"build9"' } });
  const env = assetsMock(response);
  const request = new Request(origin + "/downloads/DropEdge-Latest.dmg", { headers: { Range: "bytes=0-2", "If-Range": '"build9"' } });
  const result = await worker.fetch(request, env);
  assert.equal(result, response);
  assert.equal(env.requests[0].headers.get("Range"), "bytes=0-2");
  assert.equal(env.requests[0].headers.get("If-Range"), '"build9"');
...
test("not-modified responses do not redirect", async () => {
  const env = assetsMock(new Response(null, { status: 304, headers: { ETag: '"build9"' } }));
...
for (const source of ["../dropedge/index.html", "../dropedge/en/index.html"]) {
  test(`canonical metadata and download link resolve to the product domain in ${source}`, () => {
    ...
    assert.equal(new URL(href, origin).href, origin + "/downloads/DropEdge-Free-1.3-build9.dmg");
  });
}
